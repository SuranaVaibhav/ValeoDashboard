var donutChart = {
	...
	createDonutChart : function(containerClass, data, centerNumber, color) {
		var CenterNumber = centerNumber;
		...
		// /////////////////////////////////////////////////////////
		// If data sent from UI5 code use it or GENERATE FAKE DATA
		// /////////////////////////////////////
		// /////////////////////////////////////////////////////////
		if (data) {
			var data = data;
		...
		var vis = d3.select(containerClass).append("svg:svg").attr("width", w).attr("height", h);
		...
		var legend = center_group.append('g').attr('class', 'legend').attr('transform', function(d, i) {
			...
			return 'translate(-5,7)';
		});
		var legendRectSize = 0;
		var legendSpacing = 4;
		legend.append('text').attr('x', legendRectSize).attr('y', legendRectSize).text(function(d) {
			return CenterNumber;
		});
		// /////////////////////////////////////////////////////////
		// STREAKER CONNECTION ////////////////////////////////////
		...
		this.update = function(data, centerNumber) {
			...
			lines.enter().append("svg:line").attr("x1", 0).attr("x2", 0).attr("y1", -r - 3).attr("y2", -r - 15).attr("stroke", "gray").attr("transform", function(d) {
			...
			center_group.selectAll('text').text(function(d) {
				return CenterNumber;
			});
		}
		...
		//
		// var el = $('<label>' + dataStructure[i].label +
		// '</label>').css('left',
		// (i / vals * 100) + '%');
		//
		// $("#slider").append(el);
